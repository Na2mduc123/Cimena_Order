...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($userId) || empty($eventName) || empty($eventTime) || empty($province) || empty($district) || empty($ward) || empty($detailedAddress)) {
    ...
    try {
        ...
        $stmt = $conn->prepare("
            INSERT INTO qly_skien (user_id, event_name, event_time, logo, banner, province, district, ward, detailed_address, status)
            VALUES (:user_id, :event_name, :event_time, :logo, :banner, :province, :district, :ward, :detailed_address, 'pending')
        ");
